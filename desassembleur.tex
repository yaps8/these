...
Plusieurs choix s'offrent à qui cherche à implémenter un système d'analyse dynamique de binaire tels l'émulation, l'instrumentation et le déboguage

L'émulation consiste à lancer l'exécution dans un environnement d'exécution simulé, qui peut être un système d'exploitation complet comme c'est le cas avec TEMU\todo{cite}, le module d'analyse dynamique du projet BAP\todo{?}/BitBlaze, basé sur l'émulateur QEMU\todo{cite}.

Instrumenter un binaire consiste \todo{synomyme?} à l'exécuter en y insérant, généralement au cours de son exécution, du code assembleur servant à son analyse. Intel développe PinTools \cite{pintools} pour l'analyse des programmes tournant sur un de leurs processeurs.

Enfin le déboguage suit pas à pas l'exécution d'un programme en utilisant le drapeau Trap \todo{?} permettant de reprendre la main après chaque instruction du programme débogué afin d'examiner son environnement d'exécution.
\\

Le déboguage comme l'instrumentation n'utilisent pas de langage intermédiaire tandis qu'un émulateur tel que BAP transcrit d'abord les instructions dans son langage intermédiaire pour les exécuter avec la sémantique concrète du langage intermédiaire.
...
L'instrumentation, par rapport au déboguage, offre des performances temporelles d'exécution bien supérieures.
