...
Nous appelons GFC paramétré par cette exécution l'union des GFC parfaits de chaque de ces instantanés dans lesquel les sommets sont caractérisés par le niveau d'exécution de l'instantané, une adresse et une instruction assembleur.
...
Formellement cette définition est basée sur l'hypothèse suivante. Nous supposons que toutes les exécution du programme résultent en l'obtention du même ensemble d'instantanés de la mémoire, et donc des mêmes vagues. Plus précisément le GFC paramétré représente donc les exécution qui partagent le même découpage en vagues que l'exécution observée.
En conservant les notations prises au chapitre précédent, l'exécution d'un programme sur une entrée I est caractérisée, via l'algorithme \ref{algo:analyse_dyn_vagues}, par le couple $(T, S)$ où $T$ est une trace et $S$ une liste d'instantanés d'exécution, un pour chaque niveau d'exécution.
...
\begin{defi}
 ...
 \begin{itemize}
  \item $V=\{(X , a, I),  \exists\ (T', S')\in E_C, \exists\ (i, X, D)\in T',\
$\da{D}$=a,\ $\di{D}$=I\}$
  \item $((X_1, a_1, I_1), (X_2, a_2, I_2))\in E_C$ si et seulement si $(X_1, a_1,
I_1)\ et\ (X_2, a_2, I_2)$ se suivent dans une trace : $\exists T'\in
E_C,\ i\in \BN, (i, X_1, D_1)\ et\ (i+1, X_2, D_2)\in T\ avec\
$\da{D_1}$=a_1,\ $\di{D_1}$=I_1,$~\da{D_2}$=a_2,\ $\di{D_2}$=I_2$.
 \end{itemize}
\end{defi}
\begin{defi}
...
Dans l'exemple choisi ici, toutes les exécution partagent le même découpage en vagues.
...
\paragraph{Choix de l'exécution, pertinence de l'approche ?}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=0.7\textwidth]{supports/disas_sm/ex2_parfait_amb.pdf}
\end{center}
