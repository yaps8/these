...
% \textlangle$l:goto\ l', \Theta$\textrangle\>$ \longrightarrow $ \textlangle$l',\ \Theta$ \textrangle\\ \\
% \textlangle$l:if\ b\ then\ goto\ l', \Theta$\textrangle\>$ \longrightarrow $ \textlangle$l',\ \Theta$\textrangle\ \>si $\Theta(b)=1$\\
% 									    \>$ \longrightarrow $ \textlangle$ l+1,\ \Theta$\textrangle\ \>sinon\\ \\

% \textlangle$l:cgoto\ v, \Theta$\textrangle\>$ \longrightarrow $ \textlangle$\bot,\ \Theta$\textrangle \>si $\Theta(v)=\bot$\\
%                                           \>$\longrightarrow $ \textlangle$\Theta(v),\ \Theta$\textrangle\>sinon\\ \\
\textlangle$l:goto\ x,\ \Theta$\textrangle\>$\longrightarrow $ \textlangle$\Theta_X(x),\ \Theta$\textrangle\> \\ \\% \>si $\Theta_X(x)\in\BL$ \\
% 					  \>$\longrightarrow $ \textlangle$\bot,\ \Theta$\textrangle \>sinon \\ \\
