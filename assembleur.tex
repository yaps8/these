...
% \node[state] (CODE){Code};
% \node[state, below=2cm of CODE] (DATA){Donnée};
% \node[state, below  of DATA] (MEM){MEM};
% \node[state, right=1.4cm of ASM] (IR){LI};
% \node[state, right=6cm of IR] (INST){Instruction};    
% \node[state, above=2cm of INST] (CFG){CFG};
\node[state, rectangle split, rectangle split parts=3, draw, label=, text width=5cm] (MEM){Code \nodepart{second} Données \nodepart{third} Tas \\ ~\\ ~\\ ~\\ ~\\ ~\\ ~\\ Pile};

\node[state, draw=none, right=1cm of MEM.north east] (ADR1){\small Adresses mémoire basses};
\node[state, draw=none, right=1cm of MEM.south east] (ADR2){\small Adresses mémoire hautes};
% 
...
\itodo{désassemblage parfait}
\ijym{pb de l'atteignabilité + important (jmp eax)}
\itodo{séparation code/données = déterminer l'ensemble du code atteignable}
