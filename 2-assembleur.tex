...
Prenons le programme assembleur donné en figure \ref{fig:asm_eax}. Il ne fait que modifier le registre \eax, effectue des sauts selon la valeur d'\eax\ et se termine avec une valeur pour \eax\ à 1 ou 2. L'instruction \texttt{mov eax, 3} n'est pas atteignable puisqu'elle suit un saut inconditionnel et qu'aucune autre instruction ne provoque un saut vers elle.
...
ret
\end{lstlisting}
\caption{Fonction assembleur modifiant \eax}
\label{fig:asm_eax}
\end{figure}
...
\end{center}
\caption{GFC de l'exemple de la figure \ref{fig:asm_eax}}
