...
Certains registres sont réservés, souvent par convention. Par exemple, lors d'un appel de fonction, la convention par défaut en C \todo{cite CDELCL} stipule que la valeur de retour est passée dans le registre \texttt{eax}.
...
Les heuristiques proposées portent sur la détection de fonctions assembleur \cite{KruegelRVV04}, repérables grâce à une suite d'instructions caractéristiques ;  sur des approches statistiques cherchant à déterminer la probabilité qu'un bloc mémoire soit de l'assembleur par calcul d'entropie. Une autre approche évalue la probabilité qu'une suite d'instructions soit effectivement du code en apprenant au préalable des suites d'instructions réelles lancées lors de l'exécution d'un programme \cite{KDF09}\todo{c'est la bonne citation?}.
...
% Non standard control flow can be used against a recursive disassembler so it explores
% unreachable code. For instance, the standard behavior of a binary after executing a \emph{call} at address $a$ is to push the 
% return instruction $a+1$ on the stack. Upon execution of the next \ret\ instruction, the value on top of the stack is
% popped and the control flow is transferred to it. A classical obfuscation technique is to use \emph{push $a'$} immediately
% followed by a \emph{ret}, which behaves like a direct jump to address $a'$. If this is done after a \emph{call} at address $a$,
% the control flow will never return to address $a+1$.
% \\

% These two obfuscation methods have been addressed in the litterature: Kruegel et al. \cite{KruegelRVV04} proposed a solution based on the asumption that legitimate code do not overlap.
