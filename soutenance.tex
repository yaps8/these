...
\titre{Analyse statique, obscurcissement et chevauchement de code}
...
\begin{defi}
 ...
 \begin{itemize}
  \item V : ensemble des couples (adresse, instruction) atteignables
  \item $(a, b)\in E$ ssi il existe un chemin d'exécution au sein duquel $b$ suit immédiatement $a$
 \end{itemize}
...
\begin{frame}[t]{Implémentations}
\begin{itemize}
 \item Base : 17044 programmes contenant 4452219 sites uniques
 \only<2->{\item Apprentissage : croissance linéaire}
 \only<3->{\item Détection de 5 programmes (18160 sites) en fonction du nombre de sites appris}
\end{itemize}
...
\only<3>{\includegraphics[width=0.65\textwidth]{supports/plots/plotScan-plus-GTSI.pdf}}
\end{center}
\only<3>{\vskip-1ex SIDT : $<$ 1 seconde}
% \caption{}
% \label{fig:scan}
% \end{figure}
\end{frame}
...
\begin{frame}{Similarité logicielle : Waledac et OpenSSL\footnote{Bonfante, Calvet, Marion, Sabatier, Thierry (REcon 2012, Malware 2012)}}
 Problème :
\begin{itemize}
