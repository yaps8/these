...
\titre{Analyse statique et chevauchement de code}
...
But : rendre un binaire plus difficile à analyser et désassembler
\begin{itemize}
...
\begin{itemize}
 \item On dispose de l'ensemble des traces possibles et de leur découpage en niveaux d'exécution
 \item On veut représenter les niveaux d'exécution dans le GFC
\end{itemize}
\end{frame}
...
Trace 2 & $s_1$ & $s_2$ & $s_1$ & $s_5$ & $s_6$ & Instantanés de la mémoire\\
Trace 3 & $s_1$ & $s_2$ & $s_3$ & $s_5$ & $s_6$ & \\
Trace 4 & $s_1$ & $s_2$ & $s_4$ &       & & \\
\cline{1-6}
\end{tabular}
% \end{center}
...
\begin{itemize}
 ...
 \item Algorithmes classiques (Ullmann\only<2>{\footnote{J.R. Ullmann, ``An algorithm for Subgraph Isomorphism'' (1976)}}, VF2) trop lents
\end{itemize}
On va s'inspirer du parcours en profondeur initié par Tarjan\only<2>{\footnote{R. Tarjan, ``Depth-first search and linear graph algorithms'' (1971)}}.
\end{frame}
...
\begin{itemize}
 \item Base de 17044 programmes malveillants : 4 452 219 sites uniques
%  \only<2->{
 \item Apprentissage : croissance linéaire, $<$ 6 minutes
%  }
 \only<2->{\item Détection de 5 programmes (18160 sites) en fonction du nombre de sites appris}
\end{itemize}
...
\only<2>{\includegraphics[width=0.65\textwidth]{supports/plots/plotScan-plus-GTSI.pdf}}
\end{center}
\only<2>{\vskip-1ex Découpage : $<$ 1 seconde}
% \caption{}
% \label{fig:scan}
% \end{figure}
\end{frame}

\begin{frame}{Implémentations : Waledac et OpenSSL\footnote{Bonfante, Calvet, Marion, Sabatier, Thierry (REcon 2012, Malware 2012)}}
 Problème :
\begin{itemize}
  \item Analyser un binaire : Waledac
  \item Pas d'informations de compilation (nom des fonctions)
...
%  Ullmann & $\infty$ & $\infty$ & (6739) & (24)\\
 ...
 Découpage & $<$ 1 seconde & 1.6 seconde & 6045 & 22/28\\
 \hline
\end{tabular}
% \end{center}
\end{frame}

\begin{frame}{Similarité logicielle : Duqu et Stuxnet\footnote{Bonfante, Marion, Sabatier, Thierry (SSTIC 2013, Malware 2013)}}
