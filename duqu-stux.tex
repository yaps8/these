...
Dans un second temps nous cherchons à détecter \duqu\ avant qu'il puisse infecter une machine ciblée.
...
Pour ces deux programmes malveillants, nous avons analysé la DLL (bibliothèque logicielle au format Windows, sigle pour \emph{Dynamic Link Library}) principale fois que celle-ci a été déchiffrée.
...
Un fois le fichier cible trouvé, le pilote utilise la fonction \emph{ParsePE} pour l'examiner et y retrouver l'adresse de \ZwP.
